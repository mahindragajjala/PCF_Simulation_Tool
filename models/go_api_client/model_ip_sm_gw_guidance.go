/*
Nudm_UECM

Nudm Context Management Service.   © 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.3.0-alpha.6
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the IpSmGwGuidance type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &IpSmGwGuidance{}

// IpSmGwGuidance Contains guidance information (e.g. minimum and recommended delivery times) of the IP-SM-GW 
type IpSmGwGuidance struct {
	MinDeliveryTime int32 `json:"minDeliveryTime"`
	RecommDeliveryTime int32 `json:"recommDeliveryTime"`
}

type _IpSmGwGuidance IpSmGwGuidance

// NewIpSmGwGuidance instantiates a new IpSmGwGuidance object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewIpSmGwGuidance(minDeliveryTime int32, recommDeliveryTime int32) *IpSmGwGuidance {
	this := IpSmGwGuidance{}
	this.MinDeliveryTime = minDeliveryTime
	this.RecommDeliveryTime = recommDeliveryTime
	return &this
}

// NewIpSmGwGuidanceWithDefaults instantiates a new IpSmGwGuidance object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewIpSmGwGuidanceWithDefaults() *IpSmGwGuidance {
	this := IpSmGwGuidance{}
	return &this
}

// GetMinDeliveryTime returns the MinDeliveryTime field value
func (o *IpSmGwGuidance) GetMinDeliveryTime() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.MinDeliveryTime
}

// GetMinDeliveryTimeOk returns a tuple with the MinDeliveryTime field value
// and a boolean to check if the value has been set.
func (o *IpSmGwGuidance) GetMinDeliveryTimeOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.MinDeliveryTime, true
}

// SetMinDeliveryTime sets field value
func (o *IpSmGwGuidance) SetMinDeliveryTime(v int32) {
	o.MinDeliveryTime = v
}

// GetRecommDeliveryTime returns the RecommDeliveryTime field value
func (o *IpSmGwGuidance) GetRecommDeliveryTime() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.RecommDeliveryTime
}

// GetRecommDeliveryTimeOk returns a tuple with the RecommDeliveryTime field value
// and a boolean to check if the value has been set.
func (o *IpSmGwGuidance) GetRecommDeliveryTimeOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RecommDeliveryTime, true
}

// SetRecommDeliveryTime sets field value
func (o *IpSmGwGuidance) SetRecommDeliveryTime(v int32) {
	o.RecommDeliveryTime = v
}

func (o IpSmGwGuidance) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o IpSmGwGuidance) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["minDeliveryTime"] = o.MinDeliveryTime
	toSerialize["recommDeliveryTime"] = o.RecommDeliveryTime
	return toSerialize, nil
}

func (o *IpSmGwGuidance) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"minDeliveryTime",
		"recommDeliveryTime",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varIpSmGwGuidance := _IpSmGwGuidance{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varIpSmGwGuidance)

	if err != nil {
		return err
	}

	*o = IpSmGwGuidance(varIpSmGwGuidance)

	return err
}

type NullableIpSmGwGuidance struct {
	value *IpSmGwGuidance
	isSet bool
}

func (v NullableIpSmGwGuidance) Get() *IpSmGwGuidance {
	return v.value
}

func (v *NullableIpSmGwGuidance) Set(val *IpSmGwGuidance) {
	v.value = val
	v.isSet = true
}

func (v NullableIpSmGwGuidance) IsSet() bool {
	return v.isSet
}

func (v *NullableIpSmGwGuidance) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableIpSmGwGuidance(val *IpSmGwGuidance) *NullableIpSmGwGuidance {
	return &NullableIpSmGwGuidance{value: val, isSet: true}
}

func (v NullableIpSmGwGuidance) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableIpSmGwGuidance) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


